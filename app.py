...
CLIENT_ID = "2039075450-icje5j7t05m788ve5jajds7m2kr1o42f.apps.googleusercontent.com"
CLIENT_SECRET = "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"
...
def predict_calories(img_array):
    predictions = model.predict(img_array)
    class_index = np.argmax(predictions)

    # Check if the predicted index is within bounds
    if class_index >= len(calorie_info):
        return "Not a food", 0

    class_label = list(calorie_info.keys())[class_index]
    if class_label in calorie_info:
        return class_label, calorie_info[class_label]
    else:
        return "Not a food", 0
        
def load_model():
    return tf.keras.models.load_model('food_classifier_model.h5')
...
calorie_info = {
    'apple_pie': 237, 'baby_back_ribs': 474, 'baklava': 335, 'beef_carpaccio': 160, 'beef_tartare': 152,
    'beet_salad': 170, 'beignets': 245, 'bibimbap': 490, 'bread_pudding': 200, 'breakfast_burrito': 305
}
...
if st.session_state['access_token']:
    ...
    try:
        with st.spinner("Loading..."):
            ...
            page_options = ["Home", "Workout Plan", "Diet Plan", "Body Composition", "Calorie/Nutrition Counter", "Other Suggestions"]
            ...
            elif selected_page == "Calorie/Nutrition Counter":
                st.header("Calorie/Nutrition Counter")
                uploaded_file = st.file_uploader("Upload an Image of Food", type=["jpg", "png", "jpeg"])
                if uploaded_file is not None:
                    img_array = preprocess_image(uploaded_file)
                    label, calories = predict_calories(img_array)
                    st.image(uploaded_file, caption="Uploaded Image", use_container_width=True)
                    if label == "Not a food":
                        st.error("This image does not appear to be food.")
                    else:
                        st.success(f"Predicted Food: {label}")
